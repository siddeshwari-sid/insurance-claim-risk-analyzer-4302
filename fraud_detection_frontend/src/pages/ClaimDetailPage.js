...
import { sanitizeExplanations } from "../utils/format";
...
function getExplanations(claim) {
  const exp =
    ...
    claim?.reasonCodes ||
    claim?.explanation;
  return sanitizeExplanations(exp);
}
...
export default function ClaimDetailPage() {
  ...
  const basic = useMemo(() => {
    ...
    return {
      ...
      date:
        claim?.dateOfLoss || claim?.lossDate || claim?.date || claim?.loss_date,
        ...
        <>
          <div className="grid gridCols2" style={{ marginBottom: 14 }}>
            <div className="card cardPad">
              ...
              <div style={{ marginTop: 12, display: "grid", gap: 8 }}>
                <div>
                  ...
                  <div style={{ fontWeight: 700 }}>
                    {basic.claimant ? String(basic.claimant) : "—"}
                  </div>
                </div>
                <div>
                  <span className="kpiTitle">Amount</span>
                  <div style={{ fontWeight: 700 }}>
                    {basic.amount !== undefined && basic.amount !== null
                      ? String(basic.amount)
                      : "—"}
                  </div>
                </div>
                <div>
                  ...
                  <div style={{ fontWeight: 700 }}>
                    {basic.date ? String(basic.date) : "—"}
                  </div>
                </div>
                <div>
                  <span className="kpiTitle">Policy</span>
                  <div style={{ fontWeight: 700 }}>
                    {basic.policy ? String(basic.policy) : "—"}
                  </div>
                </div>
              </div>
              ...
              <div style={{ marginTop: 12 }}>
                ...
                <div className="smallHelp">
                  Detected by comparing this claim against previously uploaded
                  claims in the in-memory store.
                </div>
                ...
                {claim?.fraudSignals ? (
                  <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
                    ...
                    >
                      ...
                      <div className="smallHelp">
                        Number of previously stored claims with the same
                        claim_id.
                      </div>
                    </div>
                  ...
                  <div className="alert" style={{ marginTop: 10 }}>
                    No cross-claim signals available for this claim (older
                    uploads may not include them).
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="card cardPad">
              ...
              <div className="smallHelp">
                These strings explain why the claim was scored at the given risk
                level (including any cross-claim signals).
              </div>
